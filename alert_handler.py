...
def initialize_oanda_client():
    ...
    try:
        ...
        if isinstance(access_token, object) and hasattr(
            access_token, "get_secret_value"
        ):
            access_token = access_token.get_secret_value()

        oanda = oandapyV20.API(
            access_token=access_token, environment=config.oanda_environment
        )
        logger.info(
            f"OANDA client initialized for {config.oanda_environment} environment"
        )
        return True
...
async def robust_oanda_request(
    request, max_retries: int = 3, initial_delay: float = 1.0
):
        ...
        try:
            ...
            if attempt == max_retries - 1:
                raise BrokerConnectionError(
                    f"OANDA request failed after {max_retries} attempts: {e}"
                )
...
async def execute_trade(payload: dict) -> tuple[bool, dict]:
    ...
    try:
        ...
        order_request = OrderCreate(accountID=config.oanda_account_id, data=order_data)
...
def set_api_components():
    ...
    try:
        ...
        api.regime_classifier = (
            alert_handler.regime_classifier if alert_handler else None
        )
        api.db_manager = db_manager
        api.backup_manager = backup_manager
        api.error_recovery = error_recovery
        api.notification_system = (
            alert_handler.notification_system if alert_handler else None
        )
        ...
        if backup_manager:
            try:
                await backup_manager.create_backup(
                    include_market_data=True, compress=True
                )
            except Exception as e:
