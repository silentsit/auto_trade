...
class Settings(BaseSettings):
    ...
    default_risk_percentage: float = Field(default=10.0)  # Default risk percentage per trade
    ...
    allocation_percent: float = Field(default=10.0)  # % of account to allocate per trade (used if mode is 'allocation')
     ...
     # Security Settings
    webhook_secret: str = Field(default="")  # HMAC secret - set via WEBHOOK_SECRET env var
    webhook_token: str = Field(default="")   # Simple token - set via WEBHOOK_TOKEN env var  
...
async def tradingview_webhook_secure(
    request: Request,
    auth_info: dict = Depends(verify_webhook_security)
):
    """
    ...
    try:
        ...
        sanitized_data = {k: v for k, v in data.items() if k not in ['secret', 'token', 'password']}
...
async def get_security_status():
    ...
    for ip, timestamps in request_history.items():
        recent_requests = [t for t in timestamps if current_time - t < 300]  # Last 5 minutes
        if recent_requests:
