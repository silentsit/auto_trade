...
# Setup logging first, so it can be used immediately
logging.basicConfig(
    level=logging.INFO,
...
# Now safely handle imports that might fail
try:
    import pandas as pd
except ImportError:
    pd = None
    logger.warning(
        "pandas module not found, some functionality may be limited")
...
try:
    ...
    KMeans = None
    logger.warning(
        "sklearn.cluster not found, some clustering functionality will be disabled")
try:
            ...
            safe_config = {
                "app": {
                    "version": get_config_value(
                        "VERSION", "1.0.0"), "environment": get_config_value(
                        "ENVIRONMENT", "production")}, "features": {
                    "enable_advanced_loss_management": get_config_value(
                        "ENABLE_ADVANCED_LOSS_MANAGEMENT", True), "enable_multi_stage_tp": get_config_value(
                            "ENABLE_MULTI_STAGE_TP", True), "enable_market_structure_analysis": get_config_value(
                                "ENABLE_MARKET_STRUCTURE_ANALYSIS", True)}, "risk": {
                                    "max_daily_loss": get_config_value(
                                        "MAX_DAILY_LOSS", 0.20), "max_risk_percentage": get_config_value(
...
trading_config = TradingConfig()  # Ensure trading_config is available globally
...
def handle_async_errors(func: Callable[P, T]) -> Callable[P, T]:
    ...
    async def wrapper(*args: P.args, **kwargs: P.kwargs) -> T:
        try:
            ...
            # Use error recovery system if available and this is not an
...
class ErrorRecoverySystem:
    ...
    def __init__(self):
        ...
        self.backoff_factor = get_config_value(
            "error_recovery", "backoff_factor", 2.0)
        self.grace_period = get_config_value(
            "error_recovery", "grace_period", 300)  # 5 minutes
        self.running = False
    ...
    async def handle_error(
        ...
        recovery_strategy = self.recovery_strategies.get(
            error_type, self.recovery_strategies["unknown"])
    ...
    async def _handle_connection_error(
        ...
        if attempt <= self.max_retries:
            ...
            logger.info(
                f"Connection error recovery: Will retry in {delay}s (attempt {attempt}/{self.max_retries})")
            ...
            logger.warning(
                f"Connection error recovery: Max retries ({self.max_retries}) reached for {request_id}")
...
class CircuitBreaker:
    ...
    def __init__(
        ...
        # Time window to count errors (seconds)
    ...
    async def record_error(self) -> bool:
        ...
        async with self._lock:
            ...
            self.error_timestamps = [
                t for t in self.error_timestamps if t >= window_start]

            # Check if threshold exceeded
            if len(self.error_timestamps) >= self.error_threshold:
                logger.warning(
                    f"Circuit breaker tripped: {len(self.error_timestamps)} errors in last {self.window_seconds} seconds")
                self.tripped = True
    ...
    async def _check_auto_reset(self) -> None:
        ...
        if current_time - self.tripped_time >= self.cooldown_seconds:
            ...
            self.error_timestamps = []
            logger.info(
                f"Circuit breaker auto-reset after {self.cooldown_seconds} seconds cooldown")
...
class Settings(BaseModel):
    ...
    oanda_token: str = Field(default="default_token", alias='OANDA_API_TOKEN')
...
# New component for managing multiple take profit targets
...
async def cleanup_stale_sessions():
    ...
    if _session is not None and _session_created is not None:
        ...
        session_age = (
            datetime.now() - _session_created).total_seconds() / 3600
...
class Position:
    ...
    def __init__(
        ...
        self.take_profit_price = float(
            take_profit_price) if take_profit_price is not None else None
        self.stop_loss_price = float(
            stop_loss_price) if stop_loss_price is not None else None
    ...
    def _calculate_initial_risk(self) -> float:
        ...
        if self.action == "BUY":
            ...
            risk_amount = (self.stop_loss_price -
    ...
    def update_pnl(self, current_price: float) -> Tuple[float, float]:
        ...
        if self.stop_loss_price is not None and self.initial_risk > 0:
            if self.action == "BUY":
                self.r_multiple = (self.current_price - self.entry_price) / \
                    (self.entry_price - self.stop_loss_price)
            else:
                self.r_multiple = (self.entry_price - self.current_price) / \
...
class Position:
    ...
    def modify_take_profit(self, new_price: float) -> bool:
        ...
        if self.closed:
            logger.warning(
                f"Cannot modify take profit for closed position {
                    self.position_id}")
            return False
        ...
        if self.action == "BUY" and new_price <= self.current_price:
            logger.warning(
                f"Invalid take profit price {new_price} for BUY position (current: {
                    self.current_price})")
            return False

        if self.action == "SELL" and new_price >= self.current_price:
            logger.warning(
                f"Invalid take profit price {new_price} for SELL position (current: {
                    self.current_price})")
            return False
        ...
        self.updated_time = datetime.now(timezone.utc)
        logger.info(
            f"Modified take profit for position {
                self.position_id} to {
    ...
    def modify_stop_loss(self, new_price: float) -> bool:
        ...
        if self.closed:
            logger.warning(
                f"Cannot modify stop loss for closed position {
                    self.position_id}")
            return False
        ...
        if self.action == "BUY" and new_price >= self.current_price:
            logger.warning(
                f"Invalid stop loss price {new_price} for BUY position (current: {
                    self.current_price})")
            return False

        if self.action == "SELL" and new_price <= self.current_price:
            logger.warning(
                f"Invalid stop loss price {new_price} for SELL position (current: {
                    self.current_price})")
            return False
        ...
        self.updated_time = datetime.now(timezone.utc)
        logger.info(
            f"Modified stop loss for position {
                self.position_id} from {old_stop} to {
    ...
    def close(self, exit_price: float, reason: str = "manual") -> float:
        ...
        if self.stop_loss_price is not None and self.initial_risk > 0:
            if self.action == "BUY":
                self.r_multiple = (self.close_price - self.entry_price) / \
                    (self.entry_price - self.stop_loss_price)
            else:
                self.r_multiple = (self.entry_price - self.close_price) / \
                    (self.stop_loss_price - self.entry_price)
        ...
        logger.info(
            f"Closed position {
                self.position_id} at {
                self.close_price} with P&L: {
                self.pnl:.2f} ({
                    self.pnl_percentage:.2f}%)")
        return self.pnl
    ...
    def close_partial(
        ...
        if percentage <= 0 or percentage > 100:
            logger.warning(
                f"Invalid percentage for partial close: {percentage}")
            return 0.0
        ...
        logger.info(
            f"Partially closed position {
                self.position_id}: {percentage}% at {exit_price} with P&L: {
                closed_pnl:.2f}")
        return closed_pnl
    ...
    def from_dict(cls, data: dict) -> 'Position':
        ...
        position.open_time = datetime.fromisoformat(
            data["open_time"]) if "open_time" in data else position.open_time
        position.updated_time = datetime.fromisoformat(
            data["updated_time"]) if "updated_time" in data else position.updated_time
...
class PositionTracker:
    ...
    async def reconcile_positions(self):
        ...
        try:
            ...
            if not success:
                logger.error(
                    f"Failed to get open positions from broker: {result}")
                return
            ...
            for pos_id, pos in list(self.positions.items()):
                if not pos.closed and pos_id not in broker_position_ids:
                    logger.warning(
                        f"Position {pos_id} exists locally but not in broker. Marking as closed.")
                    pos.close(pos.current_price, reason="reconciliation")

            logger.info(
                f"Position reconciliation complete: {len(self.positions)} positions tracked")
    ...
    async def run_background_task(self):
        """
        External method to run the background task.
        
        This is an adapter method that simply delegates to the internal _background_updates method.
        It's mainly provided for compatibility with the rest of the codebase.
        """
        logger.info("Starting position tracker background task")
        return await self._background_updates()
    ...
    async def record_position(
        ...
        if position_id in self.positions:
            logger.warning(
                f"Position {position_id} already exists, updating metadata")
        ...
        logger.info(
            f"Position recorded: {symbol} {action} {size} @ {entry_price}")
        return True
        ...
        if not position.closed:
            ...
            if position.entry_price > 0:
                position.pnl_percentage = (
                    pnl / (position.entry_price * position.initial_size)) * 100
    ...
    async def record_trade_pnl(self, pnl: float) -> None:
        ...
        if pnl > 0:
            ...
            self.daily_stats["avg_win"] = ((self.daily_stats["avg_win"] * (
                self.daily_stats["trades_win"] - 1)) + pnl) / self.daily_stats["trades_win"]
            if pnl > self.daily_stats["largest_win"]:
                self.daily_stats["largest_win"] = pnl
        elif pnl < 0:
            self.daily_stats["trades_loss"] += 1
            self.daily_stats["avg_loss"] = ((self.daily_stats["avg_loss"] * (
                self.daily_stats["trades_loss"] - 1)) + pnl) / self.daily_stats["trades_loss"]
        ...
        if self.daily_stats["trades_total"] > 0:
            self.daily_stats["win_rate"] = (
                self.daily_stats["trades_win"] / self.daily_stats["trades_total"]) * 100

        # Update profit factor
        total_profit = sum(max(
            0, self.positions[p].pnl) for p in self.positions if self.positions[p].closed)
        total_loss = sum(abs(min(0, self.positions[p].pnl))
                         for p in self.positions if self.positions[p].closed)
        if total_loss > 0:
    ...
    async def check_max_daily_loss(
            self, account_balance: float) -> Tuple[bool, float]:
        """
        ...
        max_daily_loss = get_config_value(
            "risk_management", "max_daily_loss", 20.0)
        ...
        if max_loss_reached:
            logger.warning(
                f"Maximum daily loss reached: {
                    loss_percentage:.2f}% (limit: {
                    max_daily_loss:.2f}%)")

        return max_loss_reached, loss_percentage

    async def update_position_exits(
            self,
            symbol: str,
                ...
                if not position.closed:
                    ...
                    if position.action == "BUY" and position.r_multiple >= 1.0:
                        ...
                        if position.stop_loss_price < position.entry_price:
                            ...
                            updated = True
                            logger.info(
                                f"Updated stop loss to break-even for {position.symbol} (ID: {position.position_id})")
    ...
    async def get_position_stats(
            self, symbol: str) -> Optional[Dict[str, Any]]:
        """
        ...
        if closed_positions:
            ...
            stats["avg_r_multiple"] = sum(
                p.r_multiple for p in closed_positions) / len(closed_positions)
        ...
        if open_positions:
            ...
            stats["active_position"] = {
                ...
                # Hours
                "duration": (datetime.now(timezone.utc) - active.open_time).total_seconds() / 3600
            }
    ...
    async def _background_updates(self):
        ...
        try:
            ...
            logger.error(
                f"Error in position tracker background task: {
                    str(e)}")
            logger.error(traceback.format_exc())
                ...
                with open(self.data_file, 'r') as f:
                    ...
                    for pos_data in data.get("positions", []):
                        try:
                            ...
                            self.symbols[position.symbol].append(
                                position.position_id)
                        except Exception as e:
                            logger.error(f"Error loading position: {str(e)}")

                    logger.info(
                        f"Loaded {len(self.positions)} positions from file")
            else:
    ...
    async def _save_positions(self):
        ...
        try:
            ...
            data = {
                "positions": [
                    self.positions[pos_id].to_dict() for pos_id in self.positions],
            ...
            if os.path.exists(self.stats_file):
                with open(self.stats_file, 'r') as f:
                    ...
                    if stats.get("date") == today:
                        ...
                        logger.info(
                            f"Trade statistics from {
                                stats.get('date')} ignored, using fresh stats for {today}")
            else:
...
async def get_open_positions(
        account_id: str = None) -> Tuple[bool, Dict[str, Any]]:
    ...
    try:
        ...
        if not api_token:
            ...
            if not api_token:
                try:
                    ...
                    logger.warning(
                        f"Could not load settings for OANDA API token: {
                            str(e)}")
            ...
            if not account_id:
                ...
                if not account_id:
                    try:
                        ...
                        logger.warning(
                            f"Could not load settings for OANDA account ID: {
                                str(e)}")
        ...
        async with session.get(url, headers=headers) as response:
            if response.status != 200:
                ...
                return False, {
                    "error": f"Error getting positions: {error_text}"}
...
async def get_account_info():
    ...
    try:
        ...
        async with session.get(url, headers=headers) as response:
            ...
            if response.status != 200:
                ...
                raise HTTPException(
                    ...
                    detail=f"OANDA API error: {error_msg}")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error getting account info: {
                str(e)}")
...
async def get_positions_info():
    ...
    try:
        ...
        result = {
            ...
            "broker_positions": broker_positions.get(
                "positions",
                []) if success else []}
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error getting positions info: {
                str(e)}")
...
async def execute_trade_endpoint(request: Request):
    ...
    try:
        ...
        if "symbol" not in payload:
            raise HTTPException(
                status_code=400,
                detail="Missing 'symbol' field")
        if "action" not in payload:
            raise HTTPException(
                status_code=400,
                detail="Missing 'action' field")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error executing trade: {
                str(e)}")
...
async def close_position_endpoint(request: Request):
    ...
    try:
        ...
        if "position_id" not in payload and "symbol" not in payload:
            raise HTTPException(
                status_code=400,
                detail="Missing 'position_id' or 'symbol' field")
        ...
        if position_id:
            ...
            if symbol not in position_tracker.symbols:
                raise HTTPException(
                    status_code=404,
                    detail=f"No positions found for {symbol}")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error closing position: {
                str(e)}")
...
async def get_market_info(symbol: str, timeframe: str = "1h"):
    ...
    try:
        ...
        if not market_analysis:
            raise HTTPException(status_code=500,
                                detail="Market analysis not initialized")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error getting market info: {
                str(e)}")
...
async def calculate_position_size(request: Request):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error calculating position size: {
                str(e)}")
...
async def get_portfolio_heat():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error getting portfolio heat: {
                str(e)}")
...
async def evaluate_trade_timing(request: Request):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error evaluating trade timing: {
                str(e)}")
...
async def add_key_level(request: Request):
    ...
    try:
        ...
        if level_type not in ["support", "resistance"]:
            raise HTTPException(
                status_code=400,
                detail="Level type must be 'support' or 'resistance'")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error adding key level: {
                str(e)}")
...
async def get_error_status():
    ...
    try:
        ...
        if not error_recovery_system:
            raise HTTPException(status_code=500,
                                detail="Error recovery system not initialized")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error getting error status: {
                str(e)}")
...
class RiskManager:
    ...
    def __init__(self):
        ...
        self.default_risk_per_trade = get_config_value(
            "risk", "default_risk_percentage", 2.0)
        self.max_risk_per_trade = get_config_value(
            "risk", "max_risk_percentage", 5.0)
        self.max_portfolio_heat = get_config_value(
            "risk", "max_portfolio_heat", 15.0)
        self.max_correlated_exposure = get_config_value(
            "risk", "max_correlated_exposure", 10.0)
        ...
        try:
            if os.path.exists("risk_data.json"):
                with open("risk_data.json", "r") as f:
                    ...
                    self.symbol_correlations = risk_data.get(
                        "correlations", {})
    ...
    async def calculate_position_size(
        ...
        logger.info(
            f"Calculated position size for {symbol}: {position_size} based on {risk_percentage}% risk")
        return position_size

    @handle_async_errors
    async def check_portfolio_heat(
            self, positions: Dict[str, Position]) -> Tuple[bool, float]:
        """
        ...
        total_risk = sum(
            position.risk_percentage for position in positions.values())
        is_acceptable = total_risk <= self.max_portfolio_heat

        if not is_acceptable:
            logger.warning(
                f"Portfolio heat {total_risk}% exceeds maximum {
    ...
    async def get_volatility_adjustment(self, symbol: str) -> float:
        ...
        # Lower volatility = can allow slightly larger position size (capped at
        # 1.25x)
        else:
...
class MarketAnalysis:
    ...
    async def identify_trend(
        ...
        for tf in timeframes:
            if tf in symbol_trends:
                ...
                direction_value = 1 if direction == "up" else (
        ...
        logger.info(
            f"Trend analysis for {symbol}: {trend_direction} with strength {
                normalized_strength:.2f}")
    ...
    async def analyze_key_levels(
        ...
        resistances = [
            level for level in levels if level["type"] == "resistance"]
    ...
    async def update_key_level(
        ...
        for level in self.key_levels[symbol]:
            if level["type"] == level_type and abs(
                    level["price"] - price) < tolerance:
                # Update existing level
                level["price"] = price  # Update slightly
                level["strength"] = max(
                    level["strength"], strength)  # Increase strength
    ...
    async def get_current_market_session(self) -> Dict[str, Any]:
        ...
        result = {
            ...
            "high_liquidity": any(
                data["liquidity"] == "high" for data in active_sessions.values()),
            "high_volatility": any(
                data["volatility"] == "high" for data in active_sessions.values()),
        }
    ...
    async def evaluate_execution_timing(
        ...
        # Price will be filled by caller
        key_level_analysis = await self.analyze_key_levels(symbol, 0.0)
...
class LorentzianClassifier:
    ...
    def _calculate_fractal_dimension(self, prices: List[float]) -> float:
        ...
        dimension = 1 + (math.log(curve_length) -
                         math.log(end_to_end)) / math.log(len(prices))
    ...
    def _classify_regime(self) -> None:
        ...
        if dimension < 1.3:
            ...
            if volatility > 0.01:  # High volatility
                ...
                self.confidence = min(1.0, volatility * 20) * \
                    (1.3 - dimension) / 0.3
            ...
            if volatility > 0.015:  # Higher volatility
                ...
                self.confidence = min(1.0, volatility * 15) * \
                    (1.6 - dimension) / 0.3
            else:  # Lower volatility
                self.regime = "mixed"
                self.confidence = 0.5
            ...
            if volatility > 0.02:  # High volatility
                ...
                self.confidence = min(1.0, volatility * 10) * \
                    (dimension - 1.6) / 0.4
            else:  # Lower volatility
                self.regime = "ranging"
...
class LorentzianDistanceClassifier:
    ...
    def __init__(
        ...
        # Between 3-5 features
        self.feature_count = min(5, max(3, feature_count))
    ...
    def add_data_point(self,
        ...
        if len(self.prices) >= self.lookback_window:
            ...
            if len(
                    self.feature_history) >= 5:  # Need at least 5 points for classification
                self._calculate_distance_matrix()
                self._classify_market_condition()
                self._generate_signal()

    def _extract_features(
            self, additional_metrics: Optional[Dict[str, float]] = None) -> None:
        ...
        recent_volumes = self.volumes[-self.lookback_window:] if len(
            self.volumes) >= self.lookback_window else None
        ...
        price_roc_short = (recent_prices[-1] / recent_prices[-short_period]
                           ) - 1 if len(recent_prices) >= short_period else 0
        price_roc_medium = (recent_prices[-1] / recent_prices[-medium_period]
                            ) - 1 if len(recent_prices) >= medium_period else 0
        price_roc_long = (recent_prices[-1] / recent_prices[-long_period]
                          ) - 1 if len(recent_prices) >= long_period else 0

        # Normalized momentum feature
        momentum = 0.5 * price_roc_short + 0.3 * \
            price_roc_medium + 0.2 * price_roc_long
        features.append(momentum)

        # Feature 2: Volatility (normalized)
        if len(recent_returns) >= 5:
            volatility = statistics.stdev(
                recent_returns) if len(recent_returns) > 1 else 0
        ...
        if recent_volumes and len(recent_volumes) >= 5:
            ...
            vol_profile = min(
                1.0, max(-1.0, (recent_vol_avg / avg_volume) - 1.0))
        ...
        if additional_metrics and self.feature_count > 5:
            for i, (key, value) in enumerate(additional_metrics.items()):
                if i + 5 < self.feature_count:  # Only add up to feature_count
                    ...
                    # Here we just take the raw value, but in practice you'd
                    # normalize
    ...
    def _classify_market_condition(self) -> None:
        ...
        # Get latest feature vector and calculate similarities to regime
        # prototypes
        ...
        for regime, characteristics in regimes.items():
            ...
            similarity = 1.0 - (0.4 * momentum_diff + 0.3 *
                                volatility_diff + 0.3 * consistency_diff)
    ...
    def _generate_signal(self) -> None:
        ...
        if current_regime == "trending_up":
            ...
            trend_consistency = recent_regimes.count(
                "trending_up") / len(recent_regimes)
            ...
            # Check for consistency in recent regimes
            trend_consistency = recent_regimes.count(
                "trending_down") / len(recent_regimes)
            strength *= (0.5 + 0.5 * trend_consistency)

        elif current_regime == "ranging":
            # Ranging market - potential mean reversion
            if len(self.feature_history) >= 10:
                # Get recent mean reversion potential (feature 4 or 2 depending
                # on count)
                ...
                if mean_reversion > 0.5:  # Far above average - potential sell
                    ...
                    strength = confidence * (mean_reversion - 0.5) * 2.0

                elif mean_reversion < -0.5:  # Far below average - potential buy
                    signal = "buy"
                    # Scale 0.5-1.0 to 0.0-1.0
                    strength = confidence * (abs(mean_reversion) - 0.5) * 2.0
    ...
    def get_signal_distribution(self, lookback: int = 10) -> Dict[str, float]:
        ...
        history = self.signal_history[-min(lookback,
    ...
    def get_trend_strength(self) -> float:
        ...
        recent_signals = self.signal_history[-min(20,
                                                  len(self.signal_history)):]
...
class VolatilityMonitor:
    ...
    def _calculate_atr(self, symbol: str) -> None:
        ...
        if self.closes[symbol] and self.closes[symbol][-1] > 0:
            self.atr_percent[symbol] = (
                self.atr[symbol] / self.closes[symbol][-1]) * 100
        else:
            self.atr_percent[symbol] = 0.0

        # Store in history
        self.atr_history[symbol].append(self.atr[symbol])
        if len(self.atr_history[symbol]
               ) > 100:  # Keep up to 100 values for percentile
            self.atr_history[symbol].pop(0)

    def _calculate_historical_volatility(self, symbol: str) -> None:
        """Calculate historical volatility (standard deviation of returns)"""
        if symbol not in self.closes or len(
                self.closes[symbol]) < self.lookback_period:
            self.historical_volatility[symbol] = 0.0
            return
        ...
        for i in range(1, len(self.closes[symbol])):
            if self.closes[symbol][i - 1] > 0:
                ret = math.log(self.closes[symbol]
                               [i] / self.closes[symbol][i - 1])
                returns.append(ret)

        # Calculate standard deviation of returns
        if len(returns) >= 2:
            self.historical_volatility[symbol] = statistics.stdev(
                returns) * math.sqrt(252)  # Annualized
        else:
            self.historical_volatility[symbol] = 0.0
    ...
    def _update_volatility_percentile(self, symbol: str) -> None:
        ...
        below_current = sum(
            1 for a in self.atr_history[symbol] if a <= self.atr[symbol])
        self.volatility_percentile[symbol] = below_current / \
            len(self.atr_history[symbol])
    ...
    def get_stop_distance(self, symbol: str, price: float) -> float:
        ...
        if self.volatility_percentile[symbol] > 0.8:
            base_distance *= 1.5
    ...
    def get_risk_adjustment(self, symbol: str) -> float:
        ...
        # Above average volatility
            ...
            # 125% of normal size (can take slightly larger position)
            return 1.25
        else:
...
class AdvancedPositionSizer:
    ...
    def calculate_position_size(self,
        ...
        kelly_factor = 1.0
        if self.apply_kelly_criterion and (
                self.win_count + self.loss_count) >= 10:
        ...
        if volatility_state is not None:
            volatility_factor = volatility_state.get(
                "suggested_adjustment", 1.0)
        ...
        if risk_manager is not None and hasattr(
                risk_manager, "get_correlation_factor"):
            correlation_factor = risk_manager.get_correlation_factor(
                instrument)
        ...
        if self.current_portfolio_risk + \
                (risk_percentage / 100) > self.max_portfolio_risk:
        ...
        if self.position_size_rounding:
            ...
            position_size = self._round_to_standard_size(
                position_size, standard_lot)
    ...
    def _convert_to_pips(
            self,
            price_distance: float,
            instrument_type: str) -> float:
    ...
    def _calculate_kelly_fraction(self) -> float:
        ...
        avg_win = sum(self.win_amounts) / \
            len(self.win_amounts) if self.win_amounts else 0
        avg_loss = sum(self.loss_amounts) / \
            len(self.loss_amounts) if self.loss_amounts else 0
    ...
    def remove_closed_position(self, position_risk: float) -> None:
        ...
        self.current_portfolio_risk = max(
            0, self.current_portfolio_risk - position_risk)
    ...
    def suggest_take_profit_levels(self,
        ...
        r_multiples = timeframe_r_multiples.get(
            timeframe, {"tp1": 1.5, "tp2": 3.0, "tp3": 5.0})
...
class TimeBasedExitManager:
    ...
    def _adjust_max_duration(
            self, position: Dict[str, Any], current_time: datetime) -> None:
        ...
        # Default, would be determined by symbol prefix in real system
        instrument_type = "FOREX"

        # Detect if symbol is crypto (for demo purposes)
        if symbol.startswith("BTC") or symbol.startswith(
                "ETH") or "USD" in symbol:
        ...
        if self.is_weekend(current_time) and instrument_type != "CRYPTO":
            ...
            time_to_sunday = (6 - current_time.weekday()
                              ) % 7  # Days until Sunday
            if time_to_sunday <= 1:  # Friday or Saturday
                position["max_duration"] = min(
                    position["max_duration"], 8)  # Max 8 hours
        ...
        if self.session_adjustments:
            ...
            if current_session == "london_new_york" and instrument_type == "FOREX":
                # High volatility overlap - reduce duration for mean reversion
                # strategies
                if "MEAN_REVERSION" in position.get("strategy_type", ""):
                    position["max_duration"] = min(
                        position["max_duration"], 6)  # Max 6 hours

            elif current_session == "quiet":
                # Low volatility period - reduce duration for all positions
                position["max_duration"] = max(
                    4, position["max_duration"] * 0.7)
    ...
    def check_time_exits(self,
                         ...
                         market_prices: Optional[Dict[str,
                                                      float]] = None) -> List[Dict[str,
                                                                                   Any]]:
        """
        ...
        for position_id, position in list(self.positions.items()):
            ...
            time_in_market = (
                current_time - entry_time).total_seconds() / 3600  # Hours
            ...
            # Check if position has been open for >80% of max duration but no
            # notification yet
...
async def setup_initial_dependencies():
    ...
    # Check if initialize() method exists, otherwise try start()
    if hasattr(
    ...
    elif hasattr(market_structure_analyzer, 'start') and callable(getattr(market_structure_analyzer, 'start')):
        await market_structure_analyzer.start()
    else:
        logger.warning(
            "MarketStructureAnalyzer has no initialize() or start() method, skipping initialization")

    # Initialize multi-stage take profit manager
    multi_stage_tp_manager = MultiStageTakeProfitManager(
        position_tracker=position_tracker)
    # Check if initialize() method exists, otherwise try start()
    if hasattr(
        multi_stage_tp_manager,
        'initialize') and callable(
        getattr(
            multi_stage_tp_manager,
            'initialize')):
        await multi_stage_tp_manager.initialize()
    elif hasattr(multi_stage_tp_manager, 'start') and callable(getattr(multi_stage_tp_manager, 'start')):
        await multi_stage_tp_manager.start()
    else:
        logger.warning(
            "MultiStageTakeProfitManager has no initialize() or start() method, skipping initialization")
    ...
    if not position_tracker:
        logger.warning(
            "Position tracker not initialized, skipping advanced loss management initialization")
        ...
        try:
            ...
            if hasattr(
                ...
                logger.warning(
                    "AdvancedLossManagement has no start() method, skipping initialization")
        except Exception as e:
            logger.error(
                f"Error initializing advanced loss management: {
                    str(e)}")
            logger.error(traceback.format_exc())
            advanced_loss_management = None
            ...
            if position_tracker and position_tracker.initialized:
                ...
                for symbol, symbol_positions in positions.items():
                    try:
                        ...
                        # Placeholder - in real implementation, get actual
                        ...
                        for position in symbol_positions:
                            try:
                                ...
                                if exit_result.get("executed", False):
                                    logger.info(
                                        f"Position {position_id} exited: {
                                            exit_result.get('reason')} at price {
                                            exit_result.get('price')}")

                            except Exception as e:
                                logger.error(
                                    f"Error checking exits for position {position_id}: {
                                        str(e)}")
                                logger.error(traceback.format_exc())
                    except Exception as e:
                        logger.error(
                            f"Error updating positions for {symbol}: {
                                str(e)}")
                        logger.error(traceback.format_exc())
...
def start():
    ...
    try:
        ...
        logger.info(
            "Using default configuration (load_config_from_file not available)")
    ...
    # Add CORS middleware
    allowed_origins = get_config_value(
        "system", "allowed_origins", "*").split(",")
    app.add_middleware(
        CORSMiddleware,
    ...
    try:
        ...
        return JSONResponse(
            status_code=500, content={
                "error": f"Error getting take profit performance: {
                    str(e)}"})
...
class AlertHandler:
    ...
    def __init__(self):
        ...
        self.tradingview_templates = {
            ...
            "modify": {
                # modify_type: "sl", "tp", "both"
                "required_fields": ["symbol", "modify_type"],
                ...
                try:
                    ...
                    if position_tracker and position_tracker.initialized:
                        ...
                        for symbol, symbol_positions in positions.items():
                            # Get current price
                            try:
                                # This would be replaced with actual price
                                # fetching logic
                                ...
                                for position_id, position_data in symbol_positions.items():
                                    try:
                                        ...
                                        position_type = position_data.get(
                                            'direction', 'buy')
                                        timeframe = position_data.get(
                                            'timeframe', '1h')
                                        ...
                                        if entry_time:
                                            time_in_market = (
                                                datetime.now(
                                                    timezone.utc) - entry_time).total_seconds() / 3600  # Hours

                                            # Check if time-based exit is
                                            # needed via time_exit_manager
                                            if time_exit_manager:
                                                ...
                                                if exit_check:
                                                    ...
                                                    logger.info(
                                                        f"Time-based exit triggered for {symbol} ({position_id})")
                                                    await self._close_position(position_id=position_id, reason="time_exit")
                                    except Exception as e:
                                        logger.error(
                                            f"Error processing position {position_id}: {
                                                str(e)}")
                            except Exception as e:
                                logger.error(
    ...
    async def _handle_position_actions(
            self, position_id: str, actions: Dict[str, Any], current_price: float):
    ...
    async def _close_position(self, position_id: str, reason: str = "signal"):
        ...
        try:
            ...
            if success:
                logger.info(
                    f"Closed position {position_id} ({symbol}) with reason: {reason}")

                # If using time-based exit manager, remove from tracking
                if time_exit_manager:
                    time_exit_manager.remove_position(position_id)

                # If using multi-stage take profit manager, remove from
                # tracking
                if multi_stage_tp_manager:
    ...
    async def _close_partial_position(
        ...
        try:
            if percentage <= 0 or percentage >= 100:
                logger.error(
                    f"Invalid partial close percentage: {percentage}%")
                return False
            ...
            if not position:
                logger.warning(
                    f"Position {position_id} not found for partial closing")
                return False
            ...
            if position_obj:
                ...
                position_obj.close_partial(
                    current_price, percentage / 100.0, reason)
                logger.info(
                    f"Closed {percentage}% of position {position_id} ({symbol})")
                return True
            else:
            ...
            logger.error(
                f"Error in partial close of position {position_id}: {
                    str(e)}")
            logger.error(traceback.format_exc())
            return False
    ...
    async def _process_entry_alert(self, alert_data: Dict[str, Any]) -> bool:
        ...
        if success:
            logger.info(
                f"New position {position_id} ({symbol}) created from alert")
...
async def tradingview_webhook(
    ...
    try:
        ...
        logger.info(
            f"Received TradingView alert from {client_ip}: {
...
async def get_circuit_breaker_status():
    ...
    try:
        ...
        return JSONResponse(
            status_code=500, content={
                "error": f"Error getting circuit breaker status: {
                    str(e)}"})
...
class CorrelationAnalyzer:
    ...
    async def update_price_data(self, symbol: str, price: float) -> None:
        ...
        async with self._lock:
            ...
            time_since_update = (
                datetime.now(
                    timezone.utc) - self.last_update_time[symbol]).total_seconds() / 3600
    ...
    async def calculate_correlation(self, symbol1: str, symbol2: str) -> float:
        ...
        async with self._lock:
            ...
            try:
                ...
                logger.error(
                    f"Error calculating correlation between {symbol1} and {symbol2}: {
                        str(e)}")
                return 0.0
    ...
    async def update_correlation_matrix(self) -> None:
        ...
        async with self._lock:
            ...
            # Log update
            logger.info(
                f"Updated correlation matrix for {
                    len(symbols)} symbols, found {
                    len(new_correlated_pairs)} correlated pairs")

    async def get_correlated_symbols(
            self, target_symbol: str, threshold: Optional[float] = None) -> List[Tuple[str, float]]:
        """
    ...
    async def get_portfolio_correlation_risk(
            self, positions: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        for symbol in symbols:
            ...
            correlated_in_portfolio = [
                (sym,
                 corr) for sym,
                corr in correlated if sym in symbols and sym not in processed_symbols]
            ...
            if correlated_in_portfolio:
                ...
                processed_symbols.update(
                    [sym for sym, _ in correlated_in_portfolio])
            else:
                processed_symbols.add(symbol)
    ...
    def get_position_weight_adjustment(self,
                                       symbol: str,
                                       direction: str,
                                       existing_positions: Dict[str,
                                                                Dict[str,
                                                                     Any]]) -> float:
        """
        ...
        for pos_id, pos_data in existing_positions.items():
            ...
            if exist_symbol and exist_symbol != symbol and exist_symbol in self.correlation_matrix[
                    symbol]:
                correlation = self.correlation_matrix[symbol][exist_symbol]

                # Check direction for positive/negative correlation impact
                direction_multiplier = 1.0
                if (direction == 'buy' and exist_direction == 'buy') or (
                        direction == 'sell' and exist_direction == 'sell'):
                    # Same direction - positive correlation increases risk,
                    # negative reduces it
                    direction_multiplier = 1.0
                else:
                    # Opposite direction - positive correlation reduces risk
                    # (hedging), negative increases it
                    direction_multiplier = -1.0
                ...
                if abs(adjusted_correlation) >= self.correlation_threshold * 0.8:
                    correlated_positions.append(
                        (exist_symbol, adjusted_correlation))

        if not correlated_positions:
            return adjustment

        # Calculate adjustment based on correlation strength and number of
        # correlated positions
        correlation_adjustment = 0.0

        for _, correlation in correlated_positions:
            # Negative adjusted correlation (after direction multiplier) means
            # higher risk
            if correlation < 0:
                correlation_adjustment += abs(correlation) * 0.1
            # Positive adjusted correlation means lower risk (diversification
            # benefit)
            else:
...
async def get_symbol_correlations(symbol: str, threshold: float = None):
    ...
    try:
        ...
        return JSONResponse(
            status_code=500, content={
                "error": f"Error getting correlations for {symbol}: {
                    str(e)}"})
...
async def get_portfolio_correlation_risk():
    ...
    try:
        ...
        return JSONResponse(
            status_code=500, content={
                "error": f"Error getting portfolio correlation risk: {
                    str(e)}"})
...
async def initialize_exits(request: Request):
    ...
    try:
        ...
        return JSONResponse(
            ...
            content={
                "success": True,
                "message": f"Exit strategies initialized for position {
                    data['position_id']}",
...
async def get_exit_performance(strategy_type: Optional[str] = None):
    ...
    try:
        ...
        return JSONResponse(
            status_code=500, content={
                "error": f"Error getting exit performance metrics: {
                    str(e)}"})
...
async def get_trading_config(section: Optional[str] = None):
    ...
    try:
        ...
        # Check if trading_config is defined in globals
        if 'trading_config' not in globals():
            # Create a simple TradingConfig instance if it's not defined
            global trading_config
            trading_config = TradingConfig()
            logger.info("Created new TradingConfig instance")

        if section:
            # Try to get the section from trading_config if available
            if hasattr(trading_config, 'get_section'):
            ...
            # Get full config
            if hasattr(trading_config, 'get_config_summary'):
...
async def update_config_section(section: str, request: Request):
    ...
    try:
        ...
        if hasattr(trading_config, 'set_section'):
        ...
        return JSONResponse(
            status_code=500, content={
                "error": f"Error updating configuration section: {
                    str(e)}"})
...
async def change_exchange(exchange_id: str):
    ...
    try:
        ...
        if exchange_id.lower() == "oanda":
            ...
            if not hasattr(
                    settings,
                    "binance_api_key") or not hasattr(
                    settings,
                    "binance_api_secret"):
                return JSONResponse(
                    status_code=400, content={
                        "error": "Binance credentials not configured. Please add binance_api_key and binance_api_secret to settings."})
...
async def configure_backtest(
    ...
    try:
        ...
        return {
            "status": "success",
            "message": "Backtest configuration updated"}
...
async def list_strategies():
    ...
    try:
        strategies = [{"id": "sma_crossover",
                       "name": "SMA Crossover",
                       "description": "Simple moving average crossover strategy"},
                      {"id": "bollinger_bands",
                       "name": "Bollinger Bands",
                       "description": "Trading using Bollinger Bands mean reversion"},
                      {"id": "macd",
                       "name": "MACD",
...
async def bollinger_bands_strategy(
        exchange: BacktestExchangeAdapter,
...
async def import_csv_data(request: Request):
    ...
    try:
        ...
        # This needs to be handled with file upload, but for now just accept a
...
async def delete_historical_data(filename: str):
    ...
    try:
        ...
        if not filename.endswith(
                ".csv") or "/" in filename or "\\" in filename:
...
async def get_available_symbols():
    ...
    try:
        ...
        for symbol in symbols:
            symbols_with_timeframes[symbol] = data_downloader.get_available_timeframes(
                symbol)
...
async def optimize_strategy_parameters(
    ...
    try:
        ...
        if len(combinations) > max_combinations:
            logger.warning(
                f"Too many parameter combinations ({
                    len(combinations)}). Limiting to {max_combinations}.")
            import random
        ...
        def create_parameterized_strategy(params_dict, base_func):
            ...
            async def parameterized_strategy(exchange, current_time):
                # Add parameters to the exchange object for the strategy to
                # access
            ...
            parameterized_strategy.__name__ = f"{
                base_func.__name__}_{param_str}"
        ...
        for combo in combinations:
            params_dict = {param_names[i]: combo[i]
                           for i in range(len(param_names))}
            strategy = create_parameterized_strategy(
                params_dict, base_strategy)
        ...
        if best_strategy:
            ...
            best_index = next(
                (i for i, s in enumerate(strategies) if s.__name__ == best_strategy), -1)
            if best_index >= 0:
...
async def get_backtest_analysis(backtest_id: str,
    ...
    try:
        ...
        if not hasattr(
                backtest_engine,
                "last_backtest_id") or backtest_engine.last_backtest_id != backtest_id:
...
async def launch_backtest_dashboard():
    ...
    try:
        ...
        def run_dashboard():
            ...
            try:
                ...
                visualizer.create_dashboard(
                    host=host, port=dashboard_port, debug=False)
            except Exception as e:
        ...
        return {
            "status": "error",
            "message": f"Failed to launch dashboard: {
                str(e)}"}
