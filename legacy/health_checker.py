...
class HealthChecker:
    ...
    async def _recover_oanda_connection(self):
        ...
        try:
            ...
            # Try to reinitialize the OANDA client through the alert handler
            if hasattr(self.alert_handler, 'oanda_service') and self.alert_handler.oanda_service:
                try:
                    await self.alert_handler.oanda_service._reinitialize_client()
                    logger.info("OANDA connection recovery successful")
                    self.oanda_connection_failures = 0
                except Exception as e:
                    logger.error(f"OANDA connection recovery failed: {e}")
            else:
                logger.error("OANDA service not available for recovery")
