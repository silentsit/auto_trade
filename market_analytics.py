    ...
    async def get_market_condition(self, symbol: str, timeframe: str) -> Dict[str, Any]:
        ...
        if (symbol not in self.volatility_data or 
            ...
            if get_atr is not None:
                ...
                await self.update_volatility(symbol, current_atr, timeframe)
            else:
        ...
        # If we still don't have a timeframe, use default
        if not timeframe:
            timeframe = "1H"
            
        cache_key = f"{symbol}_{timeframe}"
        
        # Check if we have data and it's still valid
        if (symbol in self.volatility_data and 
            timeframe in self.volatility_data[symbol] and
            cache_key in self.cache_expiry and
            datetime.now() < self.cache_expiry[cache_key]):
            
            # Return cached data
            return self.volatility_data[symbol][timeframe]
            
        # Otherwise, get current ATR and update
        try:
            current_atr = await get_atr(symbol, timeframe)
            
            # Update volatility data
            condition = await self.update_volatility(symbol, current_atr, timeframe)
            
            return condition
            
        except Exception as e:
            logger.error(f"Error getting market condition for {symbol} on {timeframe}: {str(e)}")
