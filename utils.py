...
# ===== CUSTOM EXCEPTION =====
# FIX: Define the missing custom exception class.
...
def _get_simulated_price(symbol: str, side: str) -> float:
    # ... (function body as defined in the previous step)
    base_prices = {
...
# FIX: Define the missing get_instrument_leverage function.
def get_instrument_leverage(symbol: str) -> float:
    ...
    return settings.get("max_leverage", 50.0) # Default to 50x leverage if not found

# FIX: Define the missing round_position_size function.
def round_position_size(symbol: str, size: float) -> int:
    ...
    # OANDA typically requires integer units for non-CFD instruments.
    return int(round(size, 0))

# FIX: Define the missing get_position_size_limits function.
...
# FIX: Define the missing validate_trade_inputs function.
...
def get_instrument_settings(symbol: str) -> Dict[str, Any]:
    ...
    instrument_configs = {
        "EUR_USD": {"pip_value": 0.0001, "default_stop_pips": 30}, "GBP_USD": {"pip_value": 0.0001, "default_stop_pips": 40},
        "USD_JPY": {"pip_value": 0.01, "default_stop_pips": 40}, "BTC_USD": {"pip_value": 1.0, "default_stop_pips": 500, "max_leverage": 2.0},
...
# ===== EXPORT UTILITIES =====
# FIX: Update __all__ to export all the newly added functions.
__all__ = [
    'get_current_price',
